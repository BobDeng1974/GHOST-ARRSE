...
{
  mbInitialised = true;
  mirFBSize = GV3::get<ImageRef>("ARDriver.FrameBufferSize", ImageRef(512,424), SILENT);
...
void ARDriver::Render(Image<Rgb<CVD::byte> > &imFrame, SE3<> se3CfromW, bool bLost)
{
  ...
  static gvar3<int> gvnLockAR("LockAR", 0, HIDDEN | SILENT);

  //only draw 3d stuff if not lost.
  if(!bLost)
  {
    glMultMatrix(mCamera.MakeUFBLinearFrustumMatrix(0.1, 10));
	...
	if (mpMap->pGame) {
		mpMap->pGame->SetFrameSize(mirFrameSize.x, mirFrameSize.y);

		if (*gvnLockAR){
			if (bCfromW_saved){
				mpMap->pGame->Draw3D(mGLWindow, *mpMap, se3CfromW_saved);
			}
			else{
				mpMap->pGame->Draw3D(mGLWindow, *mpMap, se3CfromW);
				se3CfromW_saved = se3CfromW;
				bCfromW_saved = true;
			}
		}
		else{
	...
	else if (mpMap->pGame && *gvnLockAR && bCfromW_saved){
