...
MapSerializer::MapStatus MapSerializer::LoadMap( Map * pMap, std::string sDirName )
{
  ...
  static gvar3<std::string> gvsMapToLoad("MapToLoad", "", HIDDEN | SILENT);

  if (!(*gvsMapToLoad).empty())
	  sDirName = *gvsMapToLoad;
  ...
  if( pMap ) {
    ...
    if( msCommand == "LoadMap" )  {
      LoadMap( pMap, msDirName );
		...
		//enzo
		PTAMM::MapSerializer::MapStatus res;
		std::string msDirName_ = msDirName;
		do{
			res = SaveMap( pMap, msDirName_ );
			msDirName_ = msDirName_ + "_";
		}while(res == PTAMM::MapSerializer::MapStatus::MAP_EXISTS);
    }
  }
